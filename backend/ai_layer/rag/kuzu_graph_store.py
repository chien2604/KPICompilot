...
class KuzuGraphStore:
    ...
    def upsert_user(self, user: Any) -> None:
        self._merge("User", "id", user.id, {"full_name": user.full_name, "role": user.role, "department_id": user.department_id or 0})

    def upsert_department(self, department: Any) -> None:
        self._merge("Department", "id", department.id, {"name": department.name, "parent_id": department.parent_id or 0})

    def upsert_task(self, task: Any) -> None:
        deadline = task.deadline.isoformat() if task.deadline else ""
        self._merge("Task", "id", task.id, {"title": task.title, "status": task.status, "deadline": deadline, "department_id": task.department_id or 0})

    def upsert_evidence(self, evidence: Any) -> None:
        self._merge("Evidence", "id", evidence.id, {"file_name": evidence.file_name, "relevance_score": evidence.ai_relevance_score or 0})

    def upsert_criterion(self, criterion: Any) -> None:
        self._merge("Criterion", "id", criterion.id, {"name": criterion.criterion_name, "group_name": criterion.group_name, "max_score": criterion.max_score})

    def upsert_chunk(self, chunk: Any) -> None:
        self._merge("Chunk", "id", chunk.id, {"evidence_id": chunk.evidence_id, "content_preview": chunk.content[:240]})
    ...
    def find_task_context(self, task_id: int) -> list[dict]:
        return self._query_dicts("MATCH (t:Task)-[:TASK_HAS_EVIDENCE]->(e:Evidence) WHERE t.id=$id RETURN t.title, t.status, e.file_name, e.relevance_score LIMIT 10", {"id": task_id})

    def find_user_context(self, user_id: int) -> list[dict]:
        return self._query_dicts("MATCH (u:User)-[:USER_ASSIGNED_TASK]->(t:Task) WHERE u.id=$id RETURN u.full_name, t.title, t.status, t.deadline LIMIT 20", {"id": user_id})

    def find_department_risks(self, department_id: int | None = None) -> list[dict]:
        if department_id:
            return self._query_dicts("MATCH (d:Department)<-[:USER_BELONGS_TO]-(u:User)-[:USER_ASSIGNED_TASK]->(t:Task) WHERE d.id=$id RETURN d.name, u.full_name, t.title, t.status LIMIT 30", {"id": department_id})
        return self._query_dicts("MATCH (d:Department)<-[:USER_BELONGS_TO]-(u:User)-[:USER_ASSIGNED_TASK]->(t:Task) RETURN d.name, u.full_name, t.title, t.status LIMIT 30", {})

    def _merge(self, label: str, key: str, key_value: Any, props: dict) -> None:
        if not self.available:
            return
        set_clause = ", ".join(f"n.{name} = ${name}" for name in props)
        self._execute(f"MERGE (n:{label} {{{key}: $key_value}}) SET {set_clause}", {"key_value": key_value, **props})

    def _link(self, from_label: str, from_id: int, to_label: str, to_id: int, rel: str) -> None:
        if not self.available:
            return
        self._execute(
