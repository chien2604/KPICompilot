...
class PGVectorStore:
    """Represent p g vector store data and behavior."""

    def __init__(
        self, db_session: Session, embedding_client: BaseEmbeddingClient | None = None
    ) -> None:
        """Initialize the p g vector store."""
    ...
    def similarity_search_by_task(
        self, query_text: str, task_id: int, top_k: int = 5
    ) -> list[dict]:
        """Search similar the search by task."""

        return self.similarity_search(query_text, top_k, {"task_id": task_id})

    def similarity_search_by_user(
        self, query_text: str, user_id: int, top_k: int = 5
    ) -> list[dict]:
        """Search similar the search by user."""

        task_ids = self.db.scalars(
