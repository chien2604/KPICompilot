...
class GraphRAGService:
    """Represent graph r a g service data and behavior."""

    def __init__(self, db: Session) -> None:
        """Initialize the graph r a g service."""
    ...
    def index_evidence(self, evidence_id: int) -> dict:
        ...
        rows = self.vector_store.add_chunks(
            evidence.id, evidence.task_id, chunks, embeddings
        )
    ...
    def retrieve_for_task(self, task_id: int, query_text: str) -> dict:
        """Retrieve the for task."""

        return {
            "vectors": self.vector_store.similarity_search_by_task(query_text, task_id),
            "graph": self.graph.find_task_context(task_id),
        }

    def retrieve_for_user(self, user_id: int, query_text: str) -> dict:
        """Retrieve the for user."""

        return {
            "vectors": self.vector_store.similarity_search_by_user(query_text, user_id),
            "graph": self.graph.find_user_context(user_id),
        }

    def retrieve_for_department(self, department_id: int, query_text: str) -> dict:
        """Retrieve the for department."""

        return {
            "vectors": self.vector_store.similarity_search(query_text, 5),
            "graph": self.graph.find_department_risks(department_id),
        }
    ...
    def build_chat_context(
        ...
        return {
            "vectors": self.vector_store.similarity_search(question, 5),
            "graph": self.graph.find_department_risks(),
        }

    def _sync_graph(
        self, evidence: TaskEvidence, task: Task | None, chunks: list
    ) -> None:
