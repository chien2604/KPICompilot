...
Hỗ trợ:
  - PDF      → pdfplumber (đọc text + tables)
...
def extract_pdf(file_path: Path) -> ExtractionResult:
    """Trích xuất text và bảng từ PDF bằng pdfplumber."""
    try:
        ...
        return _result(error="pdfplumber chưa được cài. Chạy: pip install pdfplumber")
...
def extract_image(file_path: Path) -> ExtractionResult:
    ...
    try:
        ...
        return _result(error="Pillow chưa được cài. Chạy: pip install Pillow")
    except Exception as exc:  # noqa: BLE001
