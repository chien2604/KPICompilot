...
function InfoRow({ icon, label, value }) {
  /** Render one labeled personnel field. */
  return (
...
/** Render the kpi score card interface. */
function KpiScoreCard({ score }) {
  /** Render an existing KPI score without inserting fallback data. */
  if (!score)
    return <div className="profile-kpi-empty">Chưa có dữ liệu KPI</div>;
    ...
    <div className="profile-kpi-card">
      ...
      <div className="profile-kpi-circle-wrap">
        ...
        <div className="profile-kpi-circle-center">
          <span className="profile-kpi-score" style={{ color }}>
            {score.total_score}
          </span>
      ...
      <div className="profile-kpi-meta">
        ...
        <div className="profile-kpi-risk" style={{ color }}>
          <WarningOutlined />
          <span>
            Rủi ro: {riskLevelLabel[score.risk_level] || score.risk_level}
          </span>
        </div>
      </div>
    </div>
...
/** Render the personnel profile and authorized KPI history. */
export default function EmployeeProfilePage() {
  ...
  useEffect(() => {
    kpiApi.profile(userId).then(setProfile);
  }, [userId]);
    ...
    <Space direction="vertical" size={20} className="page">
      <div className="page-intro">
        <div>
          ...
          <Typography.Text>
            Hồ sơ công tác, tiến độ nhiệm vụ và kết quả KPI theo kỳ đánh giá
          </Typography.Text>
        </div>
      </div>
      ...
      <Row gutter={[20, 20]}>
            ...
            <Card className="profile-avatar-card">
              <div className="profile-avatar-wrap">
                ...
                <div className="profile-avatar-info">
                  ...
                  <div className="profile-avatar-position">
                    {user.position_title}
                  </div>
                  <Tag color="blue" style={{ marginTop: 6, fontSize: 13 }}>
                    {organizationRoleLabel[user.organization_role] ||
            ...
            >
              <div className="profile-info-list">
                ...
                <InfoRow
                  ...
                  label="Ngày vào Đảng"
                  value={user.party_joined_date}
                />
                <InfoRow
                  icon={<SafetyCertificateOutlined />}
                  label="Giáo dục phổ thông"
                  value={user.general_education}
                />
                <InfoRow
                  icon={<SafetyCertificateOutlined />}
                  label="Chuyên môn"
                ...
                <InfoRow
                  ...
                  value={user.political_theory}
                />
                <InfoRow
                  icon={<SafetyCertificateOutlined />}
                  label="Nhóm KPI"
                  value={
                    kpiTemplateLabel[user.kpi_role_template] ||
                    user.kpi_role_template
                  }
                />
                <InfoRow
                  icon={<SafetyCertificateOutlined />}
                  label="Phạm vi KPI"
                  value={user.is_kpi_eligible ? "Đang áp dụng" : "Chưa áp dụng"}
                />
                <InfoRow
                  icon={<IdcardOutlined />}
                  label="Lĩnh vực phụ trách"
                  value={(user.work_areas || [])
        ...
        <Col xs={24} lg={12}>
          <Card
            title={
              <span style={{ fontSize: 16, fontWeight: 700 }}>Điểm KPI</span>
            }
            extra={
              <DatePicker
                picker="month"
      ...
      >
        <div className="task-stats-row">
          {TASK_ITEMS.map((item) => (
            <div
              key={item.label}
              className="task-stats-item"
              style={{ background: item.bg }}
            >
              <div
                className="task-stats-item__value"
                style={{ color: item.color }}
              >
                {item.value}
              </div>
              <div
                className="task-stats-item__label"
                style={{ color: item.color }}
              >
                {item.label}
              </div>
          ...
          {FILTER_OPTIONS.map((opt) => (
            <Tag
              ...
              style={{
                ...
                background:
                  taskFilter === opt.value ? opt.color : `${opt.color}15`,
                cursor: "pointer",
                fontSize: 14,
                padding: "4px 14px",
                borderRadius: 20,
