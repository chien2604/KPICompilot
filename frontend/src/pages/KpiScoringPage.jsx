...
const STATUS_META = {
  COMPLETED:   { label: 'Hoàn thành',     color: '#16a34a', icon: <CheckCircleOutlined /> },
  IN_PROGRESS: { label: 'Đang thực hiện', color: '#f59e0b', icon: <ClockCircleOutlined /> },
  NOT_STARTED: { label: 'Chưa bắt đầu',  color: '#94a3b8', icon: <ClockCircleOutlined /> },
...
export default function KpiScoringPage() {
        ...
        prev.map((t) => {
          ...
          return {
            ...
            assignees: t.assignees.map((a) =>
              a.user_id === userId
                ? { ...a, leader_score: score, final_score: a.self_score != null ? a.self_score * 0.3 + score * 0.7 : score }
                : a,
            ),
          };
        }),
      );
      setDraftScores((d) => { const n = { ...d }; delete n[key]; return n; });
    } catch {
  ...
  // Nhóm assignable users theo phòng ban
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (s) => {
        ...
        return (
          <Tag style={{ borderRadius: 20, fontWeight: 600, borderColor: m.color, color: m.color, background: m.color + '15' }}>
            {m.icon} {m.label}
          </Tag>
        );
      },
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return <span style={{ fontWeight: 700, color: c }}>{a.final_score.toFixed(1)}</span>;
      },
    },
    {
      title: '',
    ...
    <Space direction="vertical" size={20} className="page">
      ...
      <Card>
        <Row gutter={[16, 12]} align="middle">
          <Col flex="none">
            <span style={{ fontWeight: 600, fontSize: 15 }}>Chọn cán bộ cần chấm:</span>
          </Col>
          <Col flex="auto">
            <Select
              ...
              notFoundContent={
                ...
                assignable.length === 0
                  ? <Empty description="Bạn không có quyền chấm điểm cho ai" image={Empty.PRESENTED_IMAGE_SIMPLE} />
                  : null
              }
            />
          </Col>
          <Col flex="none" style={{ color: '#64748b', fontSize: 13 }}>
            {assignable.length > 0 && `${assignable.length} người trong phạm vi quyền hạn`}
          </Col>
        </Row>
      </Card>
        ...
        <Spin spinning={loadingTasks}>
          <Space direction="vertical" size={16} style={{ width: '100%' }}>
              ...
              <Col xs={24} md={14}>
                <Card>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    />
                    <div>
                      <div style={{ fontSize: 20, fontWeight: 800 }}>{selectedUser.full_name}</div>
                      <div style={{ color: '#64748b', fontSize: 14, marginTop: 2 }}>{selectedUser.position_title}</div>
                      <div style={{ marginTop: 6 }}>
                        <Tag color={LEVEL_TAG[selectedUser.level]?.color || 'default'}>
                          {LEVEL_TAG[selectedUser.level]?.label}
                        </Tag>
                        <Tag color="default">{selectedUser.department_name}</Tag>
                      </div>
                    </div>
                  </div>
                </Card>
                  ...
                  <Row gutter={16}>
                    ...
                    <Col span={12}>
                      <Statistic
                        ...
                        valueStyle={{ color: scoreColor, fontSize: 16, fontWeight: 700 }}
                      />
                    </Col>
                  </Row>
            ...
            <div style={{
              ...
              borderRadius: 8, padding: '10px 16px', fontSize: 13, color: '#0369a1',
            }}>
              <b>Hướng dẫn:</b> Nhập điểm lãnh đạo vào cột "Điểm lãnh đạo" → nhấn <SaveOutlined /> để lưu từng nhiệm vụ → nhấn <b>"Tính lại KPI tổng hợp"</b> để cập nhật điểm KPI cuối.
              Điểm cuối = 30% tự chấm + 70% lãnh đạo chấm.
            </div>
            ...
            <Card
              title={
                <span>
                  ...
                  <Badge count={tasks.length} style={{ marginLeft: 8, background: '#6366f1' }} />
                </span>
              }
            >
              {tasks.length === 0 && !loadingTasks ? (
                ...
                <Table
                  ...
                  scroll={{ x: 700 }}
                  rowClassName={(record) => record.status === 'OVERDUE' ? 'task-row--overdue' : ''}
                />
              )}
            </Card>
