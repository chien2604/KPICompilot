      ...
      columns={[
        {
          ...
          dataIndex: 'status',
          render: (text, record) => (
            <span style={{ fontWeight: record.isTotal ? 700 : 500, color: record.color }}>
              {!record.isTotal && (
                <span style={{
                  display: 'inline-block', width: 10, height: 10,
                  borderRadius: '50%', background: record.color, marginRight: 8,
                }} />
              )}
              {text}
            </span>
        ...
        },
        {
          ...
          align: 'center',
          render: (val, record) => (
            <span style={{ fontWeight: 700, fontSize: 16, color: record.color }}>{val}</span>
          ),
        },
      ]}
    />
...
function RiskUsersTable({ riskUsers = [] }) {
  ...
  return (
    <Table
      ...
      style={{ marginBottom: 24 }}
      columns={[
        { title: 'Họ tên', dataIndex: 'name', render: (v) => <span style={{ fontWeight: 600 }}>{v}</span> },
        { title: 'Phòng ban', dataIndex: 'department' },
        {
          title: 'Điểm KPI', dataIndex: 'score', width: 100, align: 'center',
          render: (v) => <span style={{ fontWeight: 700, color: riskColor(v), fontSize: 16 }}>{v}</span>,
        },
        {
          title: 'Mức rủi ro', dataIndex: 'risk', width: 120, align: 'center',
          render: (v) => (
            <Tag color={v === 'HIGH' ? 'red' : v === 'MEDIUM' ? 'orange' : 'green'} style={{ fontSize: 13 }}>
    ...
    <div className="report-preview">
      ...
      {hasStructured && (
        <div className="report-preview__section">
          ...
          <TaskStatusTable tasksByStatus={tasksByStatus} totalTasks={totalTasks} />
        </div>
      )}
      ...
      {riskUsers.length > 0 && (
        <div className="report-preview__section">
          <div className="report-preview__section-title">Cán bộ có nguy cơ không đạt KPI</div>
